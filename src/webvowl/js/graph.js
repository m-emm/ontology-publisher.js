...
module.exports = function (graphContainerSelector) {
	 ...
	 */
	function zoomed() {
		zoomTo(d3.event.scale, d3.event.translate);
	}
	
	function zoomTo(scale,translate) {		
			graphContainer.attr("transform", "translate(" + translate + ")scale(" + scale + ")");
			// store zoom factor for export
			zoomFactor = scale;
			graphTranslation = translate;
	}
	
	function zoomToExt(scale,translate) {
		graphContainer.attr("transform", "translate(" + translate + ")scale(" + scale + ")");		
		zoomBehaviour.translate(translate).scale(scale);
		zoomFactor = scale;
		graphTranslation = translate;		
		
	}
	
	graph.zoomTo = zoomToExt;
	...
	graph.nodePositions = function() {
		...
		force.nodes().forEach(function(element){
			if(element.iri && element.type) {
				...
				var linksObject = nodePositions.links;
				if(!linksObject) {
					nodePositions.links = {};
					linksObject = nodePositions.links 
				}
				var type = element.link().property().type();
				var typeObject = linksObject[type];
				if(!typeObject) {
					typeObject = {};
					linksObject[type] = typeObject;
				}
				var posObject =  {x:element.x,y:element.y};
				if(element.pinned) {
					posObject.pinned = element.pinned();
				}
				var domainIri = element.link().domain().iri();
				var rangeIri = element.link().range().iri();
				var propertyIri = element.link().property().iri() || 'anonymous';
				
				var outgoingLinksObject = typeObject[domainIri];
				if(!outgoingLinksObject) {
					outgoingLinksObject = {};
					typeObject[domainIri] = outgoingLinksObject;
				}
				var targetLinksObject = outgoingLinksObject[propertyIri];
				if(!targetLinksObject) {
					targetLinksObject = {};
					outgoingLinksObject[propertyIri] = targetLinksObject;
				}
				targetLinksObject[rangeIri] = posObject;				
			}
		});
				...
				if(linksObject) {
					...
					if(typeObject) {
						...
						if(outgoingLinksObject) {
							var targetLinksObject = outgoingLinksObject[propertyIri];
							if(targetLinksObject) {
								var posObject = targetLinksObject[rangeIri];
								if(posObject) {
									element.x = posObject.x;
									element.y = posObject.y;
									element.px = posObject.x;
